...
int	n_words(const char *s, char *sep, int ct[2])
{
	int	qte[2];
	...
	while (s[ct[0]])
	{
		...
		{
			ct[1]++;
			while ((!ft_strchr(sep, s[ct[0]]) || qte[1]) && s[ct[0]])
			{
				if (!qte[0] && (s[ct[0]] == '\'' || s[ct[0]] == '\"'))
					qte[0] = s[ct[0]];
				qte[1] = (qte[1] + (qte[0] == s[ct[0]])) % 2;
				qte[0] *= qte[1] != 0;
				ct[0]++;
			}
			if (qte[1])
...
static char	**ft_fill_array(char *s, char **ret, char *sep, int i[3])
{
	...
	while (s[i[0]])
	{
		while (ft_strchr(sep, s[i[0]]) && s[i[0]] != '\0')
			i[0]++;
		i[1] = i[0];
		while ((!ft_strchr(sep, s[i[0]]) || qte[0] || qte[1]) && s[i[0]])
		{
