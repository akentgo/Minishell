...
static t_ms	*get_redir(t_ms *node, char **a[2], int *i)
{
	if (a[0][*i]) //if there is a command
	{
		...
		else if (a[0][*i][0] == '>' && a[0][*i + 1][0] != '>') // if we have ">" (and do not have ">>" since the last condition is false we just redirect our output to an outfile
		...
		else if (a[0][*i][0] == '<' && a[0][*i + 1][0] != '<') // if we have '<' we assign our infile
...
void	check_redir_in(char **a[2], int *i)
{
	...
	n[0] = 0;
	n[1] = 0;
	while (a[0][*i + n[0]][0] == '<' && n[0] < 3) //increase our counter n[0] to 3 as far as we find '<'
		n[0]++;
	n[1] = n[0]; //set n[1] to n[0] value
	while (a[0][*i + n[1]][0] == '<' && n[1] < 6) //from 3 to 6 we increase our counter if we find more '<'
		n[1]++;
...
void	check_redir_out(char **a[2], int *i)
{
	...
	n[0] = 0;
	n[1] = 0;
	while (a[0][*i + n[0]][0] == '>' && n[0] < 2)
		n[0]++;
	n[1] = n[0];
	while (a[0][*i + n[1]][0] == '>' && n[1] < 4)
		n[1]++;
	if (n[1] == 2)
		new_line = ft_strjoin(new_line, "newline'");
	else
